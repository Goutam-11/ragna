...
export function renderTrace(events: readonly TraceEvent[]): string {
  return events
    .map((event, index) => renderEvent(event, index + 1))
    .join("\n\n");
}
...
export function renderEvent(event: TraceEvent, sequence: number): string {
  ...
  switch (event.type) {
    ...
    case "MODEL_DECISION":
      return [`${prefix} MODEL_DECISION`, indent(event.summary)].join("\n");
    case "APPROVAL_REQUIRED":
      return [
        ...
        indent("Arguments:"),
        indent(formatValue(event.arguments), 4),
      ].join("\n");
    ...
    case "TOOL_RESULT":
      return [
        `${prefix} TOOL_RESULT`,
        indent(`Tool: ${event.tool}`),
        indent(`Result: ${event.resultKind} ${event.resultId}`),
        indent("Result:"),
        indent(formatValue(event.result), 4),
      ].join("\n");

    case "TOOL_ERROR":
      return renderToolError(event, prefix);
...
function renderToolError(
  event: Extract<TraceEvent, { type: "TOOL_ERROR" }>,
  prefix: string,
): string {
  const lines = [
    `${prefix} TOOL_ERROR`,
  ...
  if (event.details !== undefined) {
    lines.push(indent("Details:"), indent(formatValue(event.details), 4));
  }
